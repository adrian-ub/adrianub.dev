...
```ts title="rating-input.component.ts"
...
```ts title="rating-input.component.ts" /CONTROL_VALUE_ACCESSOR/
...
```html title="app.component.html" "[disabled]" "[(ngModel)]" "formControlName"
